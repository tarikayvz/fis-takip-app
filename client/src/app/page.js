...
import { 
    ...
    RefreshCw, Target, CheckCircle, Printer, Download, Save, Pencil
} from 'lucide-react'; 
import { Chart as ChartJS, CategoryScale, LinearScale, BarElement, ArcElement, Title, Tooltip, Legend } from 'chart.js';

ChartJS.register(CategoryScale, LinearScale, BarElement, ArcElement, Title, Tooltip, Legend);

// 👇 API ADRESİNİ BELİRLE (Vercel'de https://... Render'da localhost)
...
export default function Home() {
  ...
  // --- API FONKSİYONLARI (HEPSİ GÜNCELLENDİ) ---
  ...
  const handleSaveDetails = async () => {
      try {
          const res = await fetch(`${API_URL}/api/receipts/${selectedReceipt.id}`, {
              method: 'PUT',
              headers: getAuthHeaders(),
              body: JSON.stringify({
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="flex-1 md:ml-64 p-8 pb-24 transition-colors duration-300">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div><h2 className="text-2xl font-bold dark:text-white">Finansal Durum</h2><p className="text-slate-500 dark:text-slate-400 text-sm">Harcamalarını ve gelirini buradan yönet.</p></div>
