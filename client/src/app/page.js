...
import { 
    ...
    RefreshCw, Target, CheckCircle, Printer, Download, Save, Pencil, AlertTriangle
} from 'lucide-react'; 
...
export default function Home() {
  ...
  // 👇 YENİ: GÜZEL ALERT VE CONFIRM MODALLARI
  const [isAddMoneyModalOpen, setIsAddMoneyModalOpen] = useState(false); // Para Ekleme
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);   // Silme Onayı
  const [targetGoalId, setTargetGoalId] = useState(null); // Hangi hedefe para eklenecek?
  const [addAmount, setAddAmount] = useState('');         // Ne kadar eklenecek?
  const [confirmAction, setConfirmAction] = useState(null); // Silme işlemi ne olacak?
  const [confirmMessage, setConfirmMessage] = useState(''); // Silme mesajı ne?
  ...
  // SİLME İŞLEMİ (GÜZELLEŞTİRİLDİ)
  const handleDeleteRequest = (e, id, type) => {
      e.stopPropagation();
      setConfirmMessage(type === 'receipt' ? "Bu fişi silmek istediğine emin misin?" : "Hedefi silmek istiyor musun? Harcamalar iptal edilecek.");
  ...
  // HEDEFE PARA EKLEME (GÜZELLEŞTİRİLDİ)
  ...
  const submitAddMoney = async (e) => {
      ...
      if(!addAmount) return;
      try {
          await fetch(`${API_URL}/api/goals/${targetGoalId}`, { method: 'PUT', headers: getAuthHeaders(), body: JSON.stringify({ amount: parseFloat(addAmount) }) });
          fetchGoals();
          fetchReceipts();
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="flex-1 md:ml-64 p-8 pb-24 transition-colors duration-300">
        ...
        {/* HEDEF KUMBARALARI (MODAL DÜZELTİLDİ) */}
        <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {goals.map((goal) => {
                    ...
                    return (
                        // 👇 Tıklayınca openAddMoneyModal fonksiyonu çalışacak
                ...
                {filteredReceipts.map((fis) => (
                    <div key={fis.id} onClick={() => setSelectedReceipt(fis)} className={`flex justify-between items-center p-4 cursor-pointer border-b last:border-0 border-slate-100 dark:border-slate-800 transition ${fis.isCancelled ? 'bg-slate-50 dark:bg-slate-900 opacity-60 grayscale' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                        ...
                        <div className="flex items-center gap-4">
                            <span className={`font-bold ${fis.isCancelled ? 'text-slate-400 dark:text-slate-600 line-through' : 'text-slate-700 dark:text-slate-300'}`}>{fis.totalAmount} ₺</span>
                            {/* 👇 EDİT VE SİLME BUTONLARI */}
                            <button onClick={(e) => handleDeleteRequest(e, fis.id, 'receipt')} className="p-2 text-slate-400 hover:text-rose-600"><Trash2 className="w-4 h-4"/></button>
                        </div>
                    </div>
                ))}
            </div>
      ...
      {isConfirmModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm p-6 shadow-2xl border-2 border-rose-100 dark:border-rose-900">
      ...
      {/* DİĞER MODALLAR (Detay, Manuel, Hedef Ekle, Rapor) AYNI */}
