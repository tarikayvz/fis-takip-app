...
import { 
    ...
    RefreshCw, Target, CheckCircle, Printer, Download, Save, Pencil // İkonlar
} from 'lucide-react'; 
...
export default function Home() {
  ...
  const [selectedReceipt, setSelectedReceipt] = useState(null); // Hem görüntüleme hem düzenleme için
  const [searchTerm, setSearchTerm] = useState('');
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  
  // Bütçe ve Gelir (Grafikler için gerekli)
  ...
  const [isEditOpen, setIsEditOpen] = useState(false); // Düzenleme Modalı
  ...
  // --- API ÇAĞRILARI ---
  const fetchReceipts = async () => { try { const res = await fetch('/api/receipts', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }); const data = await res.json(); if(Array.isArray(data)) setReceipts(data); } catch (error) { console.error("Hata:", error); } };
  const fetchGoals = async () => { try { const res = await fetch('/api/goals', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }); const data = await res.json(); if(Array.isArray(data)) setGoals(data); } catch (error) { console.error(error); } };

  const handleDelete = async (e, id) => { e.stopPropagation(); if(!confirm("Silmek istediğine emin misin?")) return; try { const res = await fetch(`/api/receipts/${id}`, { method: 'DELETE', headers: getAuthHeaders() }); if (res.ok) { setReceipts(receipts.filter(item => item.id !== id)); if(selectedReceipt?.id === id) setSelectedReceipt(null); } } catch (error) { console.error(error); } };
  const handleUpload = async () => { if (!file) { alert("Resim seç!"); return; } setStatus('AI Analiz Ediyor... ⏳'); const formData = new FormData(); formData.append('image', file); try { const res = await fetch('/api/receipts/upload', { method: 'POST', body: formData, headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }); if (res.ok) { setStatus('Başarılı! ✅'); fetchReceipts(); setTimeout(() => setStatus(''), 3000); } else { setStatus('Hata oluştu.'); } } catch (error) { setStatus('Sunucu hatası.'); } };
  const handleManualSubmit = async (e) => { e.preventDefault(); if(!manualData.totalAmount) return alert("Tutar giriniz!"); try { const res = await fetch('/api/receipts/manual', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(manualData) }); if(res.ok) { fetchReceipts(); setIsManualOpen(false); setManualData({ merchantName: '', date: new Date().toISOString().slice(0, 10), totalAmount: '', category: 'Diğer', isRecurring: false }); setStatus('Harcama Eklendi! ✍️'); setTimeout(() => setStatus(''), 3000); } else { alert("Kaydedilemedi."); } } catch (error) { console.error(error); } };
  const handleAddGoal = async (e) => { e.preventDefault(); try { const res = await fetch('/api/goals', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(newGoalData) }); if(res.ok) { fetchGoals(); setIsGoalModalOpen(false); setNewGoalData({ title: '', targetAmount: '', color: 'bg-indigo-500' }); } } catch (error) { console.error(error); } };
  const handleUpdateGoal = async (id) => { const add = prompt("Ne kadar eklemek istiyorsun?", "0"); if(!add) return; try { await fetch(`/api/goals/${id}`, { method: 'PUT', headers: getAuthHeaders(), body: JSON.stringify({ amount: parseFloat(add) }) }); fetchGoals(); fetchReceipts(); } catch (error) { console.error(error); } };
  const handleDeleteGoal = async (e, id) => { e.stopPropagation(); if(!confirm("Hedefi silmek istiyor musun?")) return; try { await fetch(`/api/goals/${id}`, { method: 'DELETE', headers: getAuthHeaders() }); fetchGoals(); fetchReceipts(); } catch (error) { console.error(error); } };
  const handleSendMessage = async (e) => { e.preventDefault(); if (!chatInput.trim()) return; const userMessage = chatInput; setChatMessages(prev => [...prev, { role: 'user', text: userMessage }]); setChatInput(''); setChatLoading(true); try { const res = await fetch('/api/receipts/chat', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ message: userMessage }) }); const data = await res.json(); setChatMessages(prev => [...prev, { role: 'ai', text: data.reply || "Hata." }]); } catch (error) { setChatMessages(prev => [...prev, { role: 'ai', text: "Bağlantı hatası ❌" }]); } finally { setChatLoading(false); } };

  // --- DÜZENLEME (EDIT) ---
  const handleItemChange = (index, field, value) => {
      const newItems = [...selectedReceipt.items];
      newItems[index][field] = field === 'price' ? parseFloat(value) || 0 : value;
      const newTotal = newItems.reduce((acc, item) => acc + (parseFloat(item.price) || 0), 0);
      setSelectedReceipt({ ...selectedReceipt, items: newItems, totalAmount: newTotal });
  };
  const handleReceiptChange = (field, value) => { setSelectedReceipt({ ...selectedReceipt, [field]: value }); };
  const handleSaveDetails = async () => {
      try {
          const res = await fetch(`/api/receipts/${selectedReceipt.id}`, { method: 'PUT', headers: getAuthHeaders(), body: JSON.stringify({ merchantName: selectedReceipt.merchantName, date: selectedReceipt.date, totalAmount: selectedReceipt.totalAmount, category: selectedReceipt.category, items: selectedReceipt.items }) });
          if (res.ok) { alert("Güncellendi! ✅"); fetchReceipts(); setSelectedReceipt(null); } else { alert("Hata oluştu."); }
      } catch (error) { console.error(error); }
  };

  // --- FİLTRELEME & RAPOR ---
  const filteredReceipts = receipts.filter(receipt => {
    const textMatch = receipt.merchantName?.toLowerCase().includes(searchTerm.toLowerCase()) || receipt.category?.toLowerCase().includes(searchTerm.toLowerCase());
    let dateMatch = true; if (startDate && receipt.date < startDate) dateMatch = false; if (endDate && receipt.date > endDate) dateMatch = false; return textMatch && dateMatch;
  });
  const downloadExcel = () => { if (reportData.length === 0) { alert("Veri yok."); return; } const dataToExport = reportData.map(r => ({ "Tarih": r.date, "Mağaza": r.merchantName, "Kategori": r.category, "Tutar": parseFloat(r.totalAmount) })); const ws = XLSX.utils.json_to_sheet(dataToExport); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Harcamalar"); XLSX.writeFile(wb, `${reportTitle}.xlsx`); };
  const printReport = () => { window.print(); };
  const handleQuickExport = (type) => { const now = new Date(); let filtered = []; let title = ""; const getLocalDate = (d) => { const year = d.getFullYear(); const month = String(d.getMonth() + 1).padStart(2, '0'); const day = String(d.getDate()).padStart(2, '0'); return `${year}-${month}-${day}`; }; if (type === 'daily') { filtered = receipts.filter(r => r.date === getLocalDate(now)); title = "Gunluk_Rapor"; } else if (type === 'weekly') { const lastWeek = new Date(); lastWeek.setDate(now.getDate() - 7); lastWeek.setHours(0,0,0,0); filtered = receipts.filter(r => { const rDate = new Date(r.date); rDate.setHours(0,0,0,0); return rDate >= lastWeek; }); title = "Haftalik_Rapor"; } else if (type === 'monthly') { const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1); startOfMonth.setHours(0,0,0,0); filtered = receipts.filter(r => { const rDate = new Date(r.date); rDate.setHours(0,0,0,0); return rDate >= startOfMonth; }); title = "Aylik_Rapor"; } else if (type === 'yearly') { const startOfYear = new Date(now.getFullYear(), 0, 1); startOfYear.setHours(0,0,0,0); filtered = receipts.filter(r => { const rDate = new Date(r.date); rDate.setHours(0,0,0,0); return rDate >= startOfYear; }); title = "Yillik_Rapor"; } if (filtered.length === 0) alert(`⚠️ Veri bulunamadı.`); else { setReportData(filtered); setReportTitle(title); setIsReportModalOpen(true); } };
  const handleExport = (data = filteredReceipts, filenamePrefix = "Rapor") => { if (data.length === 0) { alert("Veri yok."); return; } const dataToExport = data.map(r => ({ "Tarih": r.date, "Mağaza": r.merchantName, "Kategori": r.category, "Tutar": parseFloat(r.totalAmount) })); const ws = XLSX.utils.json_to_sheet(dataToExport); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Harcamalar"); XLSX.writeFile(wb, `${filenamePrefix}.xlsx`); };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
      <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 hidden md:flex flex-col fixed h-full z-10 overflow-y-auto transition-colors duration-300">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800"><h1 className="text-2xl font-bold flex items-center gap-2 text-indigo-600 dark:text-indigo-400"><Receipt className="w-8 h-8" /> Fiş Takip</h1><p className="text-xs text-slate-400 mt-1 pl-10">Hoş Geldin, {user.name}</p></div>
        <nav className="flex-1 p-4 space-y-1">
            ...
            {/* 👇 BURASI GERİ GELDİ: FİNANS SAYFASI LİNKİ */}
      ...
      <main className="flex-1 md:ml-64 p-8 pb-24 transition-colors duration-300">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div><h2 className="text-2xl font-bold dark:text-white">Gösterge Paneli</h2><p className="text-slate-500 dark:text-slate-400 text-sm">Harcamalarını buradan analiz et.</p></div>
            <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-2 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
                <label className="cursor-pointer bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-4 py-2 rounded-lg text-sm font-medium flex gap-2 dark:text-slate-200"><UploadCloud className="w-4 h-4"/> Fiş Seç<input type="file" onChange={(e)=>setFile(e.target.files[0])} accept="image/*" className="hidden"/></label>
                <button onClick={handleUpload} className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg text-sm font-medium shadow-lg shadow-indigo-200/50">Analiz Et ✨</button>
                <button onClick={() => setIsManualOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg shadow-emerald-200/50 flex items-center gap-2"><PlusCircle className="w-4 h-4"/> Ekle</button>
            </div>
        </header>
        ...
        <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {goals.map((goal) => {
                    ...
                    return (
                        <div key={goal.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group cursor-pointer hover:border-indigo-200 dark:hover:border-indigo-800 transition" onClick={() => handleUpdateGoal(goal.id)}>
                            ...
                            <button onClick={(e) => handleDeleteGoal(e, goal.id)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition"><Trash2 className="w-4 h-4"/></button>
                        </div>
                    )
                })}
                ...
                {filteredReceipts.map((fis) => (
                    <div key={fis.id} onClick={() => setSelectedReceipt(fis)} className={`flex justify-between items-center p-4 cursor-pointer border-b last:border-0 border-slate-100 dark:border-slate-800 transition ${fis.isCancelled ? 'bg-slate-50 dark:bg-slate-900 opacity-60 grayscale' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-xs">{fis.category?.[0]}</div>
      ...
      {/* MODALS */}
      {selectedReceipt && <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelectedReceipt(null)}><div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-4xl max-h-[90vh] flex overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}><div className="w-5/12 bg-slate-100 dark:bg-slate-950 p-4 flex items-center justify-center border-r border-slate-200 dark:border-slate-800">{selectedReceipt.imagePath ? <img src={`/${selectedReceipt.imagePath.replace(/\\/g, '/')}`} className="max-h-full rounded shadow-lg"/> : <p className="text-slate-400">Resim Yok</p>}</div><div className="w-7/12 p-8 overflow-y-auto"><div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-slate-800 dark:text-white">Fiş Detayları</h2><button onClick={() => setSelectedReceipt(null)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition">✕</button></div><div className="space-y-4 mb-6"><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Mağaza</label><input type="text" value={selectedReceipt.merchantName} onChange={(e) => handleReceiptChange('merchantName', e.target.value)} className="w-full p-2 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg text-lg font-bold"/></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Tarih</label><input type="date" value={selectedReceipt.date} onChange={(e) => handleReceiptChange('date', e.target.value)} className="w-full p-2 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg"/></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Kategori</label><select value={selectedReceipt.category} onChange={(e) => handleReceiptChange('category', e.target.value)} className="w-full p-2 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg">{categories.map(c=><option key={c} value={c}>{c}</option>)}</select></div></div></div><h3 className="font-bold text-slate-700 dark:text-slate-300 mb-2 border-b dark:border-slate-800 pb-2 text-sm">ÜRÜNLER</h3><div className="space-y-2 mb-6">{selectedReceipt.items?.map((item, i) => (<div key={i} className="flex gap-2 items-center"><input type="text" value={item.name} onChange={(e) => handleItemChange(i, 'name', e.target.value)} className="flex-1 p-2 bg-slate-50 dark:bg-slate-800 border border-transparent focus:border-indigo-500 rounded text-sm dark:text-slate-200"/><input type="number" value={item.price} onChange={(e) => handleItemChange(i, 'price', e.target.value)} className="w-24 p-2 bg-slate-50 dark:bg-slate-800 border border-transparent focus:border-indigo-500 rounded text-sm font-mono text-right dark:text-slate-200"/></div>))}</div><div className="flex justify-between items-center pt-4 border-t dark:border-slate-800"><div><p className="text-xs text-slate-500">TOPLAM</p><p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{selectedReceipt.totalAmount} ₺</p></div><button onClick={handleSaveDetails} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition shadow-lg shadow-indigo-200/50"><Save className="w-4 h-4"/> Kaydet</button></div></div></div></div>}
